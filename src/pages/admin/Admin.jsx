...
function Admin() {
  ...
  const [notifiedDoctors, setNotifiedDoctors] = useState(new Set());
  ...
  }, [contract, newAdminAddress, navigate]);

  renderCountRef.current += 1;

  const handleUserRegistered = useCallback(
    (user, role, fullName) => {      
      if (role.toString() === "2" && !notifiedDoctors.has(user)) {
        setNotifiedDoctors((prev) => {
          ...
          toast.info(
            <div>
              <p>
                Bác sĩ mới đăng ký: {fullName} ({shortenAddress(user)})
              </p>
              <button
                onClick={() => handleVerifyDoctor(user)}
                className="mt-2 px-4 py-1 text-sm font-medium text-white bg-indigo-500 rounded-lg hover:bg-indigo-600"
              >
                Xác minh ngay
              </button>
            </div>,
            { autoClose: false, toastId: `doctor-${user}` }
          );
          return newSet;
        });
        fetchDoctors();
    ...
    [notifiedDoctors, fetchDoctors, handleVerifyDoctor]
  );
  ...
  useEffect(() => {
    ...
    initialize();
  }, [contract, walletAddress, navigate, handleUserRegistered, fetchDoctors]);
// contract, walletAddress, navigate, handleUserRegistered, fetchDoctors
  if (isLoading) {
    return (
              ...
              <div>
                ...
                {doctors.length === 0 ? (
                  <p className="text-gray-600 text-center">
                    Chưa có bác sĩ nào được đăng ký.
                  </p>
                ) : (
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              {!doctor.isVerified && (
                                <button
                                  onClick={() =>
                                    handleVerifyDoctor(doctor.address)
                                  }
                                  disabled={isVerifying}
